...
def exists_in_bigquery(
    event_name, event_date, event_count, channel_group, dataset_id, bq_client
):
    year = event_date[:4]
...
parser.add_argument(
    "--yesterday", action="store_true", help="Fetch data from yesterday only."
)
parser.add_argument(
    "--initial_fetch", action="store_true", help="Fetch data from a wide date range."
)
...
if args.yesterday:
    ...
    confirmation = (
        input(
            "Using the initial_fetch might result in duplicated records. Do you want to proceed? (yes/no): "
        )
        .strip()
        .lower()
    )
    ...
    sys.exit()
print(
    f"Starting fetching data from {start_date} to {valid_end_date.strftime('%Y-%m-%d')}.",
    flush=True,
)
...
if os.path.exists("token.pickle"):
    ...
    flow = Flow.from_client_secrets_file(
        CLIENT_SECRET_FILE, scopes=SCOPES, redirect_uri="http://localhost:8080/"
    )
...
def create_or_update_table_with_partition_and_cluster(
    dataset_id, simple_table_id, schema, partition_by=None, cluster_by=None
):
    ...
    try:
        ...
        print(
            f"Table {created_table.full_table_id} created or confirmed existing with specified settings."
        )
...
try:
    ...
    table.time_partitioning = bigquery.TimePartitioning(
        field=config["PARTITION_BY"], type_=bigquery.TimePartitioningType.DAY
    )
...
if all_rows_to_insert:
    errors = bq_client.insert_rows_json(
        table_id, all_rows_to_insert
    )  # Use insert_rows_json for better performance with dicts
    if errors:
        print("Errors:", errors, flush=True)
    else:
        print("Data saved to BigQuery!", flush=True)
else:
